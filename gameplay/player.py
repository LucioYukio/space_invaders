...
def movimento_nave():
    if settings.teclado.key_pressed("a") or settings.teclado.key_pressed("left") and settings.nave.x>=0:
        settings.nave.x -= 400 / (1 + settings.dificuldade * 0.3) * settings.tela.delta_time()
...
def disparo():
    ...
    if settings.teclado.key_pressed("space") and settings.tempo_atual_player>=settings.tempo_recarga_player+ (settings.dificuldade * 0.08):
